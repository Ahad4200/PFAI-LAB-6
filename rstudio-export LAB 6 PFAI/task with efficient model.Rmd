...
## Step 6: Train Random Forest Model (subsample + ranger, fixed)
...
balanced_data$Class <- factor(balanced_data$Class,
                              levels = c(0, 1),
                              labels = c("Valid", "Fraud"))

# ── Subsample to ~20k rows for Posit Cloud ───────────
set.seed(123)
n_sub     <- min(20000, nrow(balanced_data))
train_idx <- sample(nrow(balanced_data), n_sub)
sub_data  <- balanced_data[train_idx, ]

cat("Training on", nrow(sub_data), "rows out of", nrow(balanced_data), "\n")

# ── 3-fold CV, request class-probs & ROC metric ───────
library(caret)
library(pROC)    # required by twoClassSummary
ctrl <- trainControl(
  method           = "cv",
  number           = 3,
  classProbs       = TRUE,
...
# ── Tuning grid with the 3 required parameters ────────
tuneGrid <- expand.grid(
  mtry          = floor(sqrt(ncol(sub_data) - 1)),  # sqrt of predictors
  splitrule     = "gini",                           # classification
  min.node.size = 1                                 # default leaf size
)
